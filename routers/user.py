...
def register_post(request: Request, username: str = Form(...), email: EmailStr = Form(...), password: str = Form(...), db: Session = Depends(get_db)):
...
async def forgot_post(request: Request, email: EmailStr = Form(...), db: Session = Depends(get_db)):
...
"""
    message = MessageSchema(subject="🔐 Your Account Recovery Details", recipients=[email], body=body, subtype="plain")
    fm = FastMail(conf)
...
def reset_password_submit(request: Request, token: str = Form(...), new_password: str = Form(...), db: Session = Depends(get_db)):
    payload, error = decode_access_token(token)
    if error:
        return templates.TemplateResponse("reset_password.html", {"request": request, "token": token, "msg": "Invalid or expired link."})
